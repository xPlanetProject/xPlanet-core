...
describe('YouswapPair', () => {
  ...
  it('mint', async () => {
    ...
    it('addLiquidity:more', async () => {
        await addLiquidity(expandTo18Decimals(5), expandTo18Decimals(5))
        // expect(await pair.balanceOf(wallet.address)).to.eq(expandTo18Decimals(5).sub(MINIMUM_LIQUIDITY))
        await addLiquidity(expandTo18Decimals(2), expandTo18Decimals(4))
        expect(await pair.totalSupply()).to.eq(expandTo18Decimals(7))
        expect(await pair.balanceOf(wallet.address)).to.eq(expandTo18Decimals(7).sub(MINIMUM_LIQUIDITY))
        expect(await token0.balanceOf(pair.address)).to.eq(expandTo18Decimals(7))
        expect(await token1.balanceOf(pair.address)).to.eq(expandTo18Decimals(9))
    })
  ...
  it('swap:gas', async () => {
    ...
    expect(receipt.gasUsed).to.eq(60057)
  })
  ...
  it('burn', async () => {
    ...
    await expect(pair.burn(wallet.address, wallet.address, overrides))
      .to.emit(pair, 'Transfer')
